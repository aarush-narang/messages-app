...
export async function jwtMiddleware(req, res) {
    ...
    if (req.url === '/api/v1/auth/account/token') {
        await jwt.verify(token, serverRuntimeConfig.refreshTokenSecret, { algorithms: ['HS256'] }, async (err, decoded) => {
            ...
            else if (err && !decoded) throw new Error().data = { name: 'UnauthorizedError', message: 'Invalid Token', status: 401 }; // if token is invalid
...
export function generateAccessToken(user) {
    return jwt.sign(user , serverRuntimeConfig.accessTokenSecret, { expiresIn: '5s' });
}
